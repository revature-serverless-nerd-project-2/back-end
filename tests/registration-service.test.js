...
describe('Registration Tests', () => {
  test('Username provided is at least 5 characters', async () => {
    await expect(register('noway', 'password', 'password', 'test', 'revature way')).rejects.toThrow(
      LengthValidationError
    );
  });

  test('Password Provided is at least 5 characters', async () => {
    await expect(register('testuser', 'pass', 'pass', 'test', 'revature way')).rejects.toThrow(
      LengthValidationError
    );
  });
  ...
  test('Username already exists', async () => {
    ...
    await expect(
      register('testuser', 'password', 'password', 'test', 'revature way')
  ...
  test('Name field is empty', async () => {
    ...
    await expect(register('testuser', 'password', 'password', '', 'revature way')).rejects.toThrow(
      NoInputError
    );
  });
  ...
  test('Address field is empty', async () => {
    ...
    await expect(register('testuser', 'password', 'password', 'name', '')).rejects.toThrow(
      NoInputError
    );
  });
  ...
  test('Passwords do not match', async () => {
    ...
    await expect(
      register('testuser', 'password', 'password1', 'name', 'revature way')
    ).rejects.toThrow(PasswordMatchingError);
  });
